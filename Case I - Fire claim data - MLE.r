...
resGlogM <- qnorm(ufit)
Var95.GlogM <- qGlogM(0.95, sigma = as.numeric(unique(sigma)), tau = tau) # value at risk 
...
LLpareto <- function(pars, y, X){
  ...
  logL <- log(beta) + beta*log(sigma) - (beta+1)*log(y+sigma) 
  ll <- -sum(logL)
  return(ll)
}
...
# -------------------------------------------------------------
# 5. log gamma distribution
# -------------------------------------------------------------
pars <- c(0, 0)
LLlogG <- function(pars, y){
  alpha <- exp(pars[1])  
  beta <- exp(pars[2]) 
  logL <- dlgamma(y, shapelog = alpha, ratelog = beta, log = T) # using the definition in package actuar
  ll <- -sum(logL)
  return(ll)
}
